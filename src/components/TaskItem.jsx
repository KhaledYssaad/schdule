...
export default function TaskItem({ user, task, isReadOnly }) {
  ...
  const handleToggle = async () => {
    ...
    try {
      await toggleActivity({ user, id: task.id, done: !task.completed }).unwrap()
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      await updateActivity({ user, id: task.id, activity, time }).unwrap()
      setIsEditing(false)
    } catch (err) {
        ...
        {isEditing ? (
          ...
          >
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button 
                onClick={handleSave} 
                className="flex items-center gap-1 px-4 py-1.5 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg text-xs text-white font-bold shadow-lg shadow-cyan-500/20 hover:shadow-cyan-500/40 transition-all"
              >
          ...
          >
            ...
            <div className="flex-grow min-w-0 pt-0.5">
              <div className="flex items-center gap-2 mb-1">
                <Clock className={`w-3.5 h-3.5 ${task.completed ? 'text-slate-500' : 'text-cyan-400/70'}`} />
              ...
              <p className={`text-sm leading-relaxed transition-all duration-500 ${
                task.completed ? 'line-through text-slate-500 decoration-slate-600' : 'text-slate-100'
              }`}>
                {task.activity}
              </p>
            </div>
